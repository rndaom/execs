...
- **D8 (open):** The native reqwest resolver now rejects private or reserved answers for approved download hosts at direct connection time, after the bounded precheck. It delegates other hostnames so configured local/private proxy hosts remain usable. A local DNS-change fixture proves direct rebinding to loopback opens no socket; a fake CONNECT proxy fixture proves `localhost` remains reachable and receives the destination hostname. All 22 focused network tests, the full Rust workspace, Clippy, and the ignored live GitHub release-metadata smoke passed. [Hyper-util's CONNECT tunnel](https://docs.rs/hyper-util/0.1.20/src/hyper_util/client/legacy/connect/proxy/tunnel.rs.html) still lets the proxy resolve that hostname independently, so a proxied target is not bound to the prechecked address. No live CDN redirect was exercised by these fixtures; a proxy-aware target policy and its end-to-end verification are still needed before checking D8.
