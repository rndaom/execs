...
- **Hosted follow-up on direct-DNS head `a077f16`:** [all five CI jobs](https://github.com/rndaom/execs/actions/runs/35940690985) and the [Linux native rerun](https://github.com/rndaom/execs/actions/runs/35940690981) passed. The [Linux package rerun](https://github.com/rndaom/execs/actions/runs/35940690993) failed at the candidate GTK Import chooser after previous-public export and candidate launch/read; the exact ZIP path was visible but the dialog stayed mapped after one Return. A guarded, import-only harness retry passed 44 focused local package tests and was used by both upgrade cases in [current-head run 35942107825](../../design/2026-09-22-overhaul/implementation/package-smoke/run-35942107825/README.md), which passed AppImage upgrade, Debian upgrade and Debian first install. Its 45 artifact files are archived with SHA-256 provenance. The [Windows capability runs](https://github.com/rndaom/execs/actions/runs/35942107708) failed before UI export/upgrade. Follow-up traced the later inspection/cleanup refusal to a string-versus-`DateTime` process creation comparison; the first early listener snapshot was empty, and later listener state was not observed. A UTC-tick regression and harness fix pass locally; a fresh hosted run is required. These results do not close D7/D8 or the remaining release matrix.
