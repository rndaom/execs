...
- **Hosted follow-up on `dbaae5a`:** The [native Linux run](https://github.com/rndaom/execs/actions/runs/35946615400) and [unsigned Linux package run](https://github.com/rndaom/execs/actions/runs/35946615451) passed. The [Windows previous-public capability run](../../design/2026-09-22-overhaul/implementation/package-smoke/run-35946615473-windows/README.md) passed all 24 contract checks, installed and launched authentic v0.1.8, verified the private app and driver listeners on loopback, and opened the profile menu with trusted native input. It **failed before export verification** when the Export control moved during click classification; the old harness had not retained that click's event trace. Its 48 original artifact files are archived with verified SHA-256 provenance. A guarded event-time click classifier and trace persistence pass 64 focused contract tests with two Linux-only skips; a hosted rerun is still required. No Windows package or candidate-upgrade pass is claimed.
