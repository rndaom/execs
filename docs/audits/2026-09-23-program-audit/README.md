...
Native downloads use one bounded `net.rs` client with allowlisted hosts/paths, redirects, payload caps, and digest checks where available. Its [public-DNS precheck](../../../apps/desktop/src-tauri/src/net.rs#L307) and [reqwest connection](../../../apps/desktop/src-tauri/src/net.rs#L400) resolve separately, so the checked IP is not cryptographically bound to the connected IP. [Reqwest supports custom resolution](https://docs.rs/reqwest/latest/reqwest/blocking/struct.ClientBuilder.html). This is a **P3 hardening assurance gap**, not a demonstrated exploit with the current host allowlist and TLS. Any change should be regression-tested against valid CDN redirects.
