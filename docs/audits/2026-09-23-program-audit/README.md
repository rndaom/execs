...
This report records the read-only September 23 audit baseline: base commit `91f7331` with preexisting working-tree changes, browser fixtures, and the external systems checked then. The only files added during the audit were this report and its 26 browser screenshots. No release, product code, Steam installation, TF2 files, or user profile data was changed by the audit. Later fixes and their verification are tracked in [remediation.md](remediation.md); the findings below describe the original baseline.
...
The most serious baseline gap was that **profile ownership, installed files, and the content TF2 actually loads are three different things**. The app tracked files by profile and pack path, while TF2 mounts loose folders and VPKs into a shared virtual namespace. A pane could truthfully report what execs saved and still misstate the effective crosshair, HUD, viewmodel, sound, weapon script, or CFG after another pack, launch command, class CFG, or server rule wins. TF2's tracked [`gameinfo.txt`](https://github.com/SteamTracking/GameTracking-TF2/blob/master/tf/gameinfo.txt) and [Valve's Source filesystem implementation](https://github.com/ValveSoftware/source-sdk-2013/blob/master/src/public/filesystem_init.cpp) support the mount-order concern; exact server/map behavior remains conditional.

The highest-priority findings at that baseline were:
...
The audited architecture had material strengths: disk writes were centralized in core, the write lock protected live surfaces, import/parser and download paths had bounds, command parity was checked for all 88 native `call(...)` names, and the preview API was structurally tied to the native bridge. The audit found integration gaps between these parts, not a general absence of protection.
...
At the original audit baseline, `pnpm --filter @execs/desktop test` passed **923 tests in 104 files**; `pnpm --filter @execs/desktop exec tsc --noEmit` passed; `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml --workspace --locked` passed, with its existing environment-dependent ignored cases. A focused 61-test frontend selection also passed. Those results describe the modified development tree based on `91f7331`, not a packaged candidate or later remediation. This audit did not run `pnpm check`, clippy, a signed updater, Steam Cloud, retail TF2, a native screen reader, or Windows/Linux installer journeys. The [existing 0.2.0 qualification record](../../design/2026-09-22-overhaul/design-qa.md) separately leaves RND-251 and RND-324 open; this audit does not close them.
...
| CompVMInstaller, Venom, TF2Hitsounds, comfig.app sounds, cueki | On-demand third-party behavior/assets; credits and pins | The [D7 register](d7-asset-rights.md) records unresolved per-asset permissions for the CompVMInstaller animations/previews, Venom textures, TF2Hitsounds, comfig.app hosted WAVs and cueki's mod library. The pins identify reproducible sources; pin freshness does not grant usage rights. |

Native downloads use one bounded `net.rs` client with allowlisted hosts/paths, per-hop redirects, payload caps, and digest checks where available. The September 24 [draft curl transport](../../../apps/desktop/src-tauri/src/net.rs) rejects non-global destination addresses and binds direct sockets and proxy tunnels to a vetted numeric address while retaining the original URL hostname for TLS and HTTP. A bounded connection pool reuses only the same vetted origin and proxy route. The [remediation tracker](remediation.md) records 18 passing Windows and hosted Linux product loopback fixtures for HTTP/HTTPS and SOCKS5h proxies, `NO_PROXY`, redirects, certificates, body caps and deadlines. It also records the corrected Debian OpenSSL/zlib dependencies and [hosted package evidence](../../design/2026-09-22-overhaul/implementation/package-smoke/run-35963148670/README.md). D8 is checked at **34/35 audit findings** for its scoped connection-binding fix. Live CDN redirects and installed candidate upgrades remain separate release qualification checks.
