...
Native downloads use one bounded `net.rs` client with allowlisted hosts/paths, per-hop redirects, payload caps, and digest checks where available. The September 24 [draft curl transport](../../../apps/desktop/src-tauri/src/net.rs) resolves and rejects non-global destination addresses, then binds direct sockets and proxy tunnels to a vetted numeric address while retaining the original URL hostname for TLS and HTTP. A bounded connection pool reuses only the same vetted origin and proxy route. The [remediation tracker](remediation.md) records passing Windows loopback fixtures for HTTP/HTTPS and SOCKS5h proxies, `NO_PROXY`, redirects, certificates, body caps and reuse; the [transport design](d8-proxy-binding-design.md) records the earlier reqwest gap and acceptance plan. D8 remains open pending exact-head hosted and Linux proxy/package qualification. Live CDN redirects remain a separate release qualification check.
