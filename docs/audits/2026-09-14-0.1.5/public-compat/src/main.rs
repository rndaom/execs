...
const OLD_COMMIT: &str = "12bb5a8effb8aebe77c6cfc4d8cd03d9bef45cdd";
const OLD_CORE_TREE: &str = "97a40e8e7b4886e5536570a3cbea65b22736980e";
const NEW_CORE_TREE: &str = "f2d03158fd4435d931db1f55fc50da1b2ef67478";
...
fn fixture_payload() -> BTreeMap<String, Vec<u8>> {
    let mut files = BTreeMap::from([
        ("tf/cfg/config.cfg".into(), b"// v0.1.4 exporter fixture\nunbindall\nbind \"w\" \"+forward\"\nsensitivity \"2.75\"\ncl_crosshair_scale \"28\"\n".to_vec()),
...
fn main() {
    ...
    let projects = helper.parent().unwrap().parent().unwrap();
    let old_repo = projects.join("execs-014-compat");
    let new_repo = projects.join("execs-015");
    ...
    assert_eq!(
        git(&old_repo, &["rev-parse", "v0.1.4^{commit}"]),
        OLD_COMMIT
    );
    ...
    let old = old_core::profile::create_populated_profile_to(
        &old_library,
        &old_root,
        "Public 0.1.4 compatibility fixture",
        &puts,
        true,
        std::iter::empty::<&str>(),
        |manifest| {
    ...
    let public_zip = run.join("public-0.1.4.zip");
    ...
    write_json(&run.join("public-0.1.4-manifest.json"), &metadata);
    ...
    .expect("0.1.5 imports public export into empty library");
    ...
    .expect("0.1.5 imports public export beside active profile");
    ...
    let reexport_zip = run.join("reexport-0.1.5.zip");
    ...
    new_core::export_profile_to(&empty_library, &new_root, empty_id, &reexport_zip)
        .expect("0.1.5 re-exports imported profile");
    ...
    .expect("0.1.5 imports its re-export");
    ...
    let report = json!({
        "result": "PASS",
        "scope": "actual public v0.1.4 export -> integrated 0.1.5 import -> re-export -> re-import, synthetic filesystem only",
        ...
        "limits": ["Small synthetic fixture, not every public profile", "Core API verification on Windows; no packaged UI, Steam handoff, actual TF2, or installed profile library exercised"]
    });
