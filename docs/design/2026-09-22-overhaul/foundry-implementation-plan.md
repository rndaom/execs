...
| --- | --- | --- | --- | --- |
| [RND-246](https://linear.app/rndaom/issue/RND-246/clear-stale-update-offers-when-a-later-check-reports-no-update) | In Progress | authoritative updater reconciliation implemented; Windows integration passed | /root/app_settings | 32 shared settings/updater tests; integrated Windows checks passed; native pending |
| [RND-290](https://linear.app/rndaom/issue/RND-290/show-an-actionable-startup-error-when-durable-maintenance-preflight) | In Progress | native startup reporting implemented; Windows fixtures passed | /root | 15 Windows startup/content fixtures and full workspace pass; native UI pending |
| [RND-291](https://linear.app/rndaom/issue/RND-291/keep-each-settings-pane-at-a-predictable-scroll-position) | In Progress | pane scroll retention implemented; scoped browser and integration checks passed | /root/surface_inventory | 6 scroll tests and accepted browser captures; native pending |
...
| [RND-251](https://linear.app/rndaom/issue/RND-251/run-the-cumulative-windows-and-linux-functional-release-matrix-before) | In Progress | Windows automated and scoped browser qualification passed; platform matrix open | /root | Windows checks and scoped browser QA passed; native/Linux/release rows open |
| [RND-213](https://linear.app/rndaom/issue/RND-213/delete-profiles-safely-from-the-profile-menu) | In Progress | safe profile deletion implemented; Windows fixtures and integration passed | /root/profile_management | Disposable deletion/profile fixtures and full Windows integration passed |
| [RND-214](https://linear.app/rndaom/issue/RND-214/add-app-settings-for-updates-motion-install-location-and-support) | In Progress | global App settings implemented; Windows integration passed | /root/app_settings | 13 core + shared 32 frontend tests; integrated and scoped browser pass |
...
| [RND-215](https://linear.app/rndaom/issue/RND-215/enforce-one-hud-per-profile-across-replacement-import-switch-and) | In Progress | HUD ownership lifecycle and exact-byte boundaries implemented; Windows proof passed | /root/foundry_hud | 69 frontend, full Windows core/workspace and separate 6-test pinned corpus pass |
...
| [RND-208](https://linear.app/rndaom/issue/RND-208/validate-and-publish-the-combined-020-overhaul-and-profile-management) | In Progress | Foundry development candidate committed; Windows and scoped browser proof passed | /root with bounded pane agents | Committed development candidate; integrated Windows and scoped browser pass; no release |
...
**Remaining implementation/qualification:** Actual signed Windows/Linux updater discovery/install/failure/close behavior and native desktop qualification.
...
**Remaining implementation/qualification:** Linux CI/native runtime, packaged Windows/Linux startup and updater/profile compatibility, actual Cloud and approved TF2/Casual/performance matrix. A Linux-CI draft PR is planned but not created at this checkpoint.
...
**Remaining implementation/qualification:** Linux and native/offline/source-availability cases beyond the disposable Windows fixtures.
...
| Windows native | WebView2 rendered/usable; native dialogs/pickers/close, startup error without console, accessibility names/speech, actual media and process guards. | Pending; do not infer from browser fixtures. |
| Linux native | WebKitGTK rendered/usable; AppImage/.deb paths as applicable, keyboard/accessibility, media, safe unsupported viewmodel-build feedback and profile compatibility. | Pending; CI alone does not establish runtime behavior. |
| Transactions/compatibility | Disposable old/new profile fixtures; source hashes, write gate, shared content, interrupted recovery, HUD/particle ownership, exact export/import and unchanged directory VPK. | Windows workspace passes 859 tests; exact-byte HUD ownership/import/recovery boundaries and profile fixtures are included. The separate pinned HUD run passes 6 tests including exact payloads and long paths. Linux and packaged/native cases remain open. |
| Packages/updater | Realistic prior-public library round trip, visible app startup, signed discovery/install/error/stalled payload, Windows NSIS and Linux update/install distinctions. | Qualification requirement remains open; no release tag/publication authorized. |
