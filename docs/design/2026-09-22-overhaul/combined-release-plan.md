...
The [Windows previous-public capability run 35937290574](implementation/package-smoke/run-35937290574-windows/README.md) failed after a verified public v0.1.8 NSIS install. Its old app and WebView2 child launched, but no private debug listener appeared; later process identity changes were refused, and UI Automation was unavailable. The [direct-DNS rerun](https://github.com/rndaom/execs/actions/runs/35940690983) and [current-head rerun](https://github.com/rndaom/execs/actions/runs/35942107708) repeated the listener/identity failure and timed out during cleanup. No Windows export, candidate upgrade, or native UI qualification is claimed. The prior strict-mode registry enumeration failure was fixed before these runs, and the first failure evidence is retained. [RND-251](https://linear.app/rndaom/issue/RND-251/run-the-cumulative-windows-and-linux-functional-release-matrix-before) stays open for that diagnosis and the other unperformed gates. The audit remains at **33/35**; D7 permissions and D8 proxied CONNECT destination binding remain open. No release is authorized.
