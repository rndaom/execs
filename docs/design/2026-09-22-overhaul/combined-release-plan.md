...
The [next native Linux run](https://github.com/rndaom/execs/actions/runs/35946615400) and [unsigned Linux package run](https://github.com/rndaom/execs/actions/runs/35946615451) passed on `dbaae5a`. Its [Windows previous-public capability run 35946615473](implementation/package-smoke/run-35946615473-windows/README.md) passed 24/24 preflight contracts, launched authentic v0.1.8, verified app and driver listeners on loopback, and opened the profile menu with trusted native input. It **failed before export verification** when the active profile's Export control moved during click classification; that run did not retain the second click's event trace. Its 48 original artifacts are archived with verified SHA-256 provenance. The next harness change records event-time pointer boxes and persists the trace before classifying it, while still refusing a target that moves before input. Focused local contract tests pass 64 with two Linux-only skips. A hosted rerun is required; no Windows candidate installer, signed updater, native Save, or upgrade pass is claimed.
