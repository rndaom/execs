...
Draft PR #60 head **`65d20bd`** passed all five [CI jobs](https://github.com/rndaom/execs/actions/runs/35937290554). The [native Linux run 35937290549](implementation/linux-native/run-35937290549/README.md) passed both inactive and active cases, including Files draft retention, explicit Save, native close → Cancel, Discard with restart, Save-and-close with restart, and clean close. The [Linux package run 35937290557](implementation/package-smoke/run-35937290557/README.md) passed its unsigned development AppImage upgrade, Debian upgrade, and Debian first-install cases with exact fixture checks. These are development runs on the PR test merge; they are not signed updater, Cloud, or retail TF2 results.

The [Windows previous-public capability run 35937290574](implementation/package-smoke/run-35937290574-windows/README.md) failed after a verified public v0.1.8 NSIS install. Its old app and WebView2 child launched, but no private debug listener appeared; later process identity changes were refused, and UI Automation was unavailable. No Windows export, candidate upgrade, or native UI qualification is claimed. The prior strict-mode registry enumeration failure was fixed before this run, and the failure evidence is retained. [RND-251](https://linear.app/rndaom/issue/RND-251/run-the-cumulative-windows-and-linux-functional-release-matrix-before) stays open for that diagnosis and the other unperformed gates. The audit remains at **33/35**; D7 permissions and D8 proxy-aware DNS/connection binding remain open. No release is authorized.
