...
The [Windows previous-public capability run 35937290574](implementation/package-smoke/run-35937290574-windows/README.md) failed after a verified public v0.1.8 NSIS install. Its old app and WebView2 child launched; the first early snapshot had no private debug listener, and later inspection and cleanup rejected process identity. The [direct-DNS rerun](https://github.com/rndaom/execs/actions/runs/35940690983) and [later rerun](https://github.com/rndaom/execs/actions/runs/35942107708) repeated that pattern. Follow-up found a harness ISO-string-versus-`DateTime` creation-time comparison that falsely rejected the same process. The [UTC-tick fix run 35944325809](implementation/package-smoke/run-35944325809-windows/README.md) confirmed the app listener on `127.0.0.1`, then failed because EdgeDriver bound the IPv6 wildcard `::`; driver-child cleanup refused an unapproved process, and UI Automation still failed to load. Its 51 artifact files are archived with SHA-256 provenance. The [next Windows run](https://github.com/rndaom/execs/actions/runs/35945890556) stopped in a preflight console-host fixture (23/24 tests passed), before any package probe; the hosted diagnostic identified a synthetic RUNNER~1 alias versus the expanded runneradmin path. The test now uses the canonical observed path while production ownership checks stay unchanged; 24/24 local tests passed; the next hosted run is recorded below. No Windows export, candidate upgrade, or native UI qualification is claimed. [All five CI jobs](https://github.com/rndaom/execs/actions/runs/35944326001), [native Linux](https://github.com/rndaom/execs/actions/runs/35944325817) and [Linux packages](https://github.com/rndaom/execs/actions/runs/35944325738) passed on the UTC-tick fix head. The prior strict-mode registry enumeration failure was fixed before these runs, and the first failure evidence is retained. [RND-251](https://linear.app/rndaom/issue/RND-251/run-the-cumulative-windows-and-linux-functional-release-matrix-before) stays open for the Windows diagnosis and other unperformed gates. The audit remains at **33/35**; D7 permissions and D8 proxied CONNECT destination binding remain open. No release is authorized.

On `dbaae5a`, [all five CI jobs](https://github.com/rndaom/execs/actions/runs/35946615411), the [native Linux run](https://github.com/rndaom/execs/actions/runs/35946615400), and [unsigned Linux package run](https://github.com/rndaom/execs/actions/runs/35946615451) passed. Its [Windows previous-public capability run 35946615473](implementation/package-smoke/run-35946615473-windows/README.md) passed 24/24 preflight contracts, launched authentic v0.1.8, verified app and driver listeners on loopback, and opened the profile menu with trusted native input. It **failed before export verification** when Export moved during click classification; that run did not retain the second click's event trace. Its 48 original artifacts are archived with verified SHA-256 provenance. The subsequent harness change records event-time pointer boxes and persists the trace before classifying it, while still refusing a target that moves before input.

On `fac08e0`, [all five CI jobs](https://github.com/rndaom/execs/actions/runs/35947645179), [native Linux](https://github.com/rndaom/execs/actions/runs/35947645163) and [unsigned Linux packages](https://github.com/rndaom/execs/actions/runs/35947645148) passed. The [Windows previous-public run 35947645153](implementation/package-smoke/run-35947645153-windows/README.md) retained trusted pointerdown and click events on Export, then **failed before native Save or ZIP verification** because its PowerShell 7 helper could not load the .NET Framework UI Automation assembly. All 56 original artifact files are archived with verified SHA-256 provenance. The follow-up harness now uses Windows PowerShell 5.1 in STA for only Save and Close, with Framework-compatible path containment checks. All 27 focused local Windows contracts, Biome and whitespace checks pass; hosted Save/Close verification is pending. No Windows candidate installer, signed updater, or upgrade pass is claimed.

On `7e3ff44`, the [Windows previous-public run 35948836291](implementation/package-smoke/run-35948836291-windows/README.md) passed 27 hosted preflight contracts and reached the native Export Save dialog with Framework UI Automation. Its same-PID, visible `Export profile` dialog had no Win32 owner, so the harness stopped before entering the filename or invoking Save. The 51 original files are archived with verified SHA-256 provenance. The follow-up predicate accepts the one foreground process-owned top-level dialog only with the exact app PID, one matching main window and no competing native dialog; it rejects unrelated same-PID dialogs and foreign processes. Local Windows contracts pass 28/28 in both PowerShell editions, with Biome and whitespace checks passing. Hosted ZIP export and normal close remain open, as do Windows candidate install/upgrade, signed updater, Cloud and retail TF2 gates.

On `3843e12`, the [Windows previous-public run 35949580124](implementation/package-smoke/run-35949580124-windows/README.md) passed 28 hosted preflight contracts. The native Save helper accepted the exact ZIP path and returned after Save invocation, but no file appeared at that path before timeout; the final exports inventory remained empty. Its 52 original artifacts are archived with verified SHA-256 provenance. The run retained no post-invoke native dialog state, so this is an unresolved failure, not evidence of a completed export or product defect. The next harness run will record bounded owned-dialog/window and WebView observations plus four exact candidate path presences after a failed wait, with only the requested path eligible for success. Local Windows contracts pass 29/29 and static checks pass. Windows export/normal-close and all later release gates remain open.
...
The scope and acceptance below originated in the September 22 Foundry selection. The revised visual direction and current qualification checkpoint at the top of this plan supersede its design wording; the functional and release gates remain in force.
...
**Date: unset, pending implementation and qualification estimates.** Foundry was selected at this September 22 checkpoint; the September 23 direction above supersedes it. October 1 was the prior minor checkpoint, not a promise for this expanded release. Both milestone target dates were cleared during consolidation; October 1 remains only the superseded historical checkpoint. Choose the delivery date after the implementation/qualification scope is estimated. Urgent install, updater, data-loss or write-lock fixes still need prompt independent delivery if the overhaul is not ready.

The original audit workspace was `rndaom/inventory-manager` at `9ac45a3`; its captures included a separate shipped 0.1.8 Mods surface. At this checkpoint, implementation proceeded on `rndaom/foundry-overhaul`; the parent integrated the 0.1.8 forward-port at `e1fecdaaf4de2c7257eb2dbe875ac45f22ee4dcc`. Qualify the combined Mods behavior against minor-track profile-owned preloaders. Product implementation was committed locally at `7c78fbed1907dceb37bf348596fca48a8f654e66`; that checkpoint recorded passing Windows automated and scoped browser checks. Native/platform/release qualification remains open.
...
The result should feel like a polished desktop product from a major game studio: restrained TF2 character, clear hierarchy, high-quality typography and spacing, responsive controls, purposeful motion and credible operational states. The three original alternatives remain available as historical comparisons. Foundry was the September 22 selection; the revised direction at the top of this plan governs current implementation. A catalog, editor, form and visual builder should retain layouts suited to their jobs.
...
The matrix was written for the September 22 Foundry selection. Its production-pane and decision-state coverage remains required under the revised direction; reusable state patterns may share components when their application to all panes is documented. Native-only gaps in the audit may receive clearly labeled proposed designs, never fabricated current screenshots.

| Surface | Required implementation and verification coverage |
| --- | --- |
...
- The September 22 selection was Foundry only. Its five boards in `options/01-foundry/` and the source-backed corrections in [concept-review.md](concept-review.md) remain design history. The revised direction at the top of this plan governs current implementation.
...
## Historical Foundry implementation planning update — September 22, 2026

At this checkpoint, the owner selected all five Foundry boards (`01-core`, `02-customization`, `03-workspaces`, `04-profiles`, `05-states`) as the sole direction. The September 23 direction above supersedes that selection. Exact functional acceptance and source-based implementation/proof gaps for every selected issue are in [foundry-implementation-plan.md](foundry-implementation-plan.md). Known image inaccuracies remain in [concept-review.md](concept-review.md); generated metrics, game art and unsupported actions are not product requirements.

The initial selection update moved RND-208 from Todo to In Progress. The later authorized implementation-allocation update moved the remaining selected Backlog/Todo records to In Progress; all 14 are now in that state. The 25-candidate pool, assignees/labels/dependencies and shipped 0.1.8 records are preserved. No new issue is needed under the existing workspace-quota fallback. The project document and milestone/project notes recorded Foundry selection at that checkpoint; their current summaries use the revised direction. Machine-readable historical verification is in [planning-status.json](planning-status.json).
