...
- **Mods repair.** "Show me the stale files" was not enough: the pane now owns the fix. The stale entries have no stock bytes anywhere on the machine, so Repair asks Steam to verify TF2 (`steam://validate/440` through the opener plugin), polls status every 5 s until every untracked particle file reads as stock, then re-applies the selection that was on screen when Repair was pressed. Twenty-minute ceiling with an honest message; the preview fixture carries one stale file so the block is visible.
- **Sounds library.** The per-slot picker sheet is gone. The pane has one library under the two slots: a search box, a source filter pill (All / Built in / Community / comfig.app) and a sort pill (A to Z / Z to A / Source); every row plays and has Hit / Kill assign buttons (the active one rings). "Add your own WAV" puts the picked file at the top of the library. Third source: comfig.app's hits collection — the pinned `hitsounds.json` (2,272 entries, SHA-512 keyed) from the comfig-app repo (MIT), audio from `hits.comfig.app` on demand. Their CDN challenges empty or library user agents, so the app now identifies as `execs/<version> (+repo)`, which the research confirmed is the only condition that matters. Every comfig sound is 4-bit MS-ADPCM: TF2 plays it as-is, the webview cannot, so `preview_wav` decodes ADPCM to 16-bit PCM for the play button only; the installed file stays byte-identical.
- **One Casual preload.** The Viewmodels pane no longer has its own preload switch; the Mods pane's hero is now "Casual preload" with two switch rows for this profile — Preload on launch (the shared itemtest cfg + launch token, what viewmodel packs and mods both need; `set_profile_preload`) and Material bypass (the gameinfo toggle, previously an Enable/Disable button). Building a viewmodel pack or applying mods still turns preload on by itself; the stat box shows Preload / Bypass / Patched files / Addon pack.
- **Bring your own HUD.** Two buttons on the HUD pane: Import a HUD archive (zip or 7z through the same magic-sniffed extractor) and Import a HUD folder (an extracted download; junk and `sound.cache` skipped, same size caps, wrapper folder stripped). The folder name becomes the HUD id (sanitised), the record is Local with no hash, and Match to catalog can pair it with hud-db later for updates.
- **HUD sorting.** Sort pill on the catalog: A to Z, Last updated, Most downloads, Most views. hud-db has none of these, comfig-api gates its counters behind a build token, and comfig.app's own "Hype" is a composite baked at build time — so dates come from comfig.app's listing pages (`Last updated <strong>…</strong>` per card, ~20 pages, all listed HUDs) and download/view counts from tf2huds.dev's SvelteKit `__data.json` per HUD (~170 HUDs, matched to hud-db through `comfigHudsUrl`). Fetched a day at a time, never blocking the catalog; unknowns sort last in name order and the row meta shows what is known ("398k downloads · 1.2M views · updated Jan 2026"). GameBanana was left out of sounds on the user's steer.

### Verification
`tsc --noEmit`, desktop 29 files / 207 tests, cfglint 100, `pnpm check` (biome), `pnpm build`, `cargo fmt --check`, `cargo clippy --workspace --all-targets -D warnings`, core 277 + app 30 tests — all clean after the follow-up. Walked `settings-sounds` (slots + inline library with search, source and sort pills), `settings-viewmodels` (hover swaps the stage, caption and weapons line), `settings-crosshair` (stock grid, thumb chips, overrides popover), `settings-hud-installed` (sort pill, per-row counts and dates), `settings-mods` (repair block); no page console errors.
