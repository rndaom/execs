//! Private development executable; never linked into the desktop app.
mod protocol;
...
fn main() {
    if let Err(error) = run() {
        eprintln!("Inventory probe failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    let Some(path) = args.next() else {
        println!("Usage: execs-inventory-probe <absolute path to TF2's steam_api64.dll or libsteam_api.so>\nSteam must be running and signed in; TF2 must be closed.\nConnects for at most 30 seconds. Sends only ClientHello; reports aggregate counts.\nMay temporarily show you playing TF2 in Steam. No credentials or inventory files are saved.");
        return Ok(());
    };
    if args.next().is_some() || !cfg!(target_pointer_width = "64") {
        return Err("Expected one library path and a 64-bit host".into());
    }
    let path = PathBuf::from(path);
    if !path.is_absolute() || !path.is_file() {
        return Err("Provide an existing absolute path to TF2's Steam API library".into());
    }
    ...
    if path.file_name().and_then(|s| s.to_str()) != Some(expected) {
        return Err(format!("Expected {expected} from the user's installed TF2").into());
    }
...
unsafe fn connect(path: &std::path::Path, system: &mut System) -> Result<()> {
    ...
    println!("Connected to the existing Steam session (account ID withheld).");
    let get_gc =
    ...
    while started.elapsed() < Duration::from_secs(30) {
        ...
        for _ in 0..64 {
            ...
            match kind & !protocol::PROTOBUF {
                ...
                24 => {
                    snapshot = Some(protocol::summary(
                        protocol::payload(kind, &bytes)?,
                        steam_id,
                    )?);
                }
        ...
        if welcomed {
            if let Some((count, unplaced)) = snapshot {
                if !logged_on(user) || identity(user) != steam_id {
                    return Err("Account changed before completion".into());
                }
                println!("Received account-matched initial backpack: {count} items, {unplaced} unplaced.\nNo item mutations sent. This probe does not track subsequent inventory changes.");
                return Ok(());
            }
        }
