...
- First-party Casual preload (RND-164) is a listen-map precache (`map itemtest` then `disconnect`). Vanilla profiles use `+exec execs_preload`; mastercomfig profiles use `+exec overrides/execs_preload` because arbitrary cfg files under `tf/cfg/overrides/` must be addressed through that path. It does not edit `gameinfo.txt` and does not patch official VPKs. Toggleable. Remade in-house — do not vendor cueki/Pilso/Horsie preloaders.
...
- **Crosshair** — default in-game crosshair controls plus the **custom crosshair** builder (first-party shapes + user PNG; "custom crosshairs", not "per-weapon crosshairs"). The stock live preview renders the actual selected sprite geometry (extracted frame-0 shapes; scale = ×`cl_crosshair_scale`/32; `cl_crosshair_alpha` is stored but the engine draws opaque). The builder bakes an optional RGB tint into the VTFs and apply normalizes `cl_crosshair_red/green/blue` to 255 for a faithful render; weapon overrides offer an "All classes" per-slot mode and a copy-class-to-all action. Default controls keep writing the gameplay override; the builder writes `tf/custom/execs-crosshairs/`.
- **Viewmodels** — working features first (prebuilt VPK import, Casual preload, remove); compiler UI is hidden entirely — never shown-disabled — until `viewmodel_compile_capability()` flips. First-person reference imagery is real per-class/slot renders from the Official TF2 Wiki (Valve content, fan use, credited), vendored under `src/assets/viewmodels/`. File-safe FOV/min stay on Gameplay.
...
- [RND-155](https://linear.app/rndaom/issue/RND-155) Bind recorder for usual actions writes `execs_binds.cfg` (overrides or vanilla). Autoexec gets `exec execs_binds`. After absorb, tracked in-game Options binds sync into that file. No `unbindall`, no alias studio.
...
- [RND-164](https://linear.app/rndaom/issue/RND-164) Viewmodels pane: prebuilt VPK import remains supported. The in-app first-party compiler is disabled until its extraction/decompile/edit/compile pipeline is genuinely file-safe and complete; never run `studiomdl` against the live `tf/` tree or present unimplemented animation edits as working. The eventual compiler covers origin/rotate/hide/keep-visible/static/left-arm, extracts Valve animation trees from the user's local TF2 install into a versioned app-data `studio/` cache, and compiles only in an isolated staging game root. First-party Casual preload is `map itemtest` then `disconnect` via `+exec execs_preload` (vanilla) or `+exec overrides/execs_preload` (mastercomfig) — remade in-house, toggleable, no hand-edit of autoexec. Never edit `gameinfo.txt`. Never store `+quit`.
